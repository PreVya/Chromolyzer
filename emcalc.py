...
def weights_cal(u1,u2,u3):
    ...
    print(wts)
    emp_per=calculate_resilience_percentage(wts)
    if emp_per > 97:
...
def calculate_resilience_percentage(wts):
    emp_per=[]
    for i in range(3):
        tpl=wts[i]
        hue_weight=tpl[0]
...
#main
scoring_dict = {
    'Ques1': {'Red': 7, 'Orange': 6, 'Yellow':5, 'Green':4, 'Blue': 3, 'Purple':2,'Pink': 1},
    'Ques2': {'Red': 7, 'Orange': 6, 'Yellow':5, 'Green':4, 'Blue': 3, 'Purple':2,'Pink': 1},
    'Ques3': {'Red': 7, 'Orange': 6, 'Yellow':5, 'Green':4, 'Blue': 3, 'Purple':2,'Pink': 1}
} 
ques_wt={
    'Ques1':1,
    'Ques2':2,
